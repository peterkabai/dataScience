...
## System Design - The Significants {.build}
...
* Can continuous predictors be used?
* Yes, either by binning or by reading the value from the density function.
* What assumptions do we make when using Naive Bayes?
* The predictors are conditionally independent.
* What assumptions do we make using NB in R?
* The distribution is Gaussian.
* What other thing do wee need to pay attention to, when multiplying fractions?
* Underflow, which can be prevented by taking the logs.
...
If we just want to count the occurrences in each category, we can use the `table()` function.
How would you create a table of `cyl` values, and plot it using a bar plot?
...
What's the probability of rolling higher than a 4, given you rolled higher than a 3?

`2/3` because there are 3 possibilities higher than 3 (4, 5, and 6) and of those, 2 are higher than 4.
